...
import { variants } from '@/components/motion/animations'
...
export interface GallerySliderProps {
  ...
  galleryClass?: string
  navigation?: boolean
  onImageClick?: (index: number, image: StaticImageData | string) => void
  clickable?: boolean
}
...
export default function GallerySlider({
  ...
  const handleImageClick = () => {
    if (clickable && onImageClick) {
      onImageClick(index, currentImage || '')
    }
  }
  ...
  return (
    ...
    >
      <div className={` group group/cardGallerySlider ${className}`} {...handlers}>
        {/* Main image */}
        <div className={`w-full overflow-hidden ${galleryClass}`}>
          <div
            className={`flex items-center justify-center ${ratioClass} ${clickable ? 'cursor-pointer' : ''}`}
